...
//echo '<style type="text/css">@import url(../../revolution/share/jscripts/jscalendar/calendar-blue.css);</style>';
//echo '<script type="text/javascript" src="../../revolution/share/jscripts/jscalendar/calendar.js"></script>';
//echo '<script  type="text/javascript" src="../../revolution/share/jscripts/jscalendar/lang/calendar-en.js"></script>';
//echo '<script type="text/javascript" src="../../revolution/share/jscripts/jscalendar/calendar-setup.js"></script>';
echo '<style type="text/css">@import url(../../revolution/share/jscripts/jscalendar/calendar-blue.css);</style>';
echo '<script type="text/javascript" src="../../revolution/share/jscripts/jscalendar/calendar.js"></script>';
echo '<script  type="text/javascript" src="../../revolution/share/jscripts/jscalendar/lang/calendar-en.js"></script>';
echo '<script type="text/javascript" src="../../revolution/share/jscripts/jscalendar/calendar-setup.js"></script>';
...
if (isset($_REQUEST['college'])) {
	$college = htmlspecialchars($_REQUEST['college'], ENT_COMPAT, $l['a_meta_charset']);
} else {
	$college = '';
}
...
if (isset($_REQUEST['user_Chinaname'])) {
	$user_Chinaname = htmlspecialchars($_REQUEST['user_Chinaname'], ENT_COMPAT, $l['a_meta_charset']);
} else {
	$user_Chinaname = '';
}
	...
	if($formstatus = F_check_form_fields()) { // check submitted form fields
		...
		if ($formstatus) {
			...
			$sql = 'INSERT INTO '.K_TABLE_USERS.' (
				...
				'.F_empty_to_null($user_Chinaname).',
				...
				'.F_empty_to_null($teacher_id).',
				'.F_empty_to_null($class_id).',
				'.F_empty_to_null($college).'
				)';
			...
			echo '<strong><a href="/erpExam_0.2/index.php" title="'.$l['h_index'].'">'.$l['h_index'].' &gt;</a></strong>'.K_NEWLINE;
			echo '</div>'.K_NEWLINE;
...
$regfields['user_lastname'] = 0;
$regfields['user_firstname'] = 0;
$regfields['user_Chinaname']=2;
$regfields['college']=2;
echo '<div class="container">'.K_NEWLINE;
...
//}
	...
	echo getFormRowTextInput('user_Chinaname',"姓名", "姓名", '', $user_Chinaname, '', 255, false, false, false, showRequiredField($regfields['user_firstname']));
...
if ($regfields['user_groups']) {
	...
	echo '<select name="user_groups[]" id="user_groups" size="1">'.K_NEWLINE;
	$sql = "SELECT *
		FROM ".K_TABLE_GROUPS." where group_name ='考生'";
	if($r = F_db_query($sql, $db)) {
 ...
 */
   	$sql_class = "SELECT *
		FROM flow_class";
	echo '<div class="row">'.K_NEWLINE;
	echo '<span class="label">班级'.K_NEWLINE;
	echo showRequiredField($regfields['user_groups']);
	echo "</span>".K_NEWLINE;
	echo '<span class="formw">'.K_NEWLINE;
	echo '<select name="class_id">'.K_NEWLINE;
	if($r = F_db_query($sql_class, $db)) {
		while($m = F_db_fetch_array($r)) {
			echo "<option value='".$m["class_id"]."'>".$m["class_name"]."</option>";
		}
	}
	echo '</select>'.K_NEWLINE;	
	echo '</div>'.K_NEWLINE;
 ...
 */	
   	$sql_class = "SELECT *
		FROM flow_teachers";
	echo '<div class="row">'.K_NEWLINE;
	echo '<span class="label">任课老师'.K_NEWLINE;
	echo showRequiredField($regfields['user_groups']);
	echo "</span>".K_NEWLINE;
	echo '<span class="formw">'.K_NEWLINE;
	echo '<select name="teacher_id">'.K_NEWLINE;
	if($r = F_db_query($sql_class, $db)) {
		while($m = F_db_fetch_array($r)) {
			echo "<option value='".$m["flow_teachers_id"]."'>".$m["flow_teachers_name"]."</option>";
		}
	}
	echo '</select>'.K_NEWLINE;	
	echo '</div>'.K_NEWLINE;
 ...
 * 显示学校
 */	
	echo getFormRowTextInput('college', '学校', '学校', '', $college, '', 255, false, false, false, showRequiredField($regfields['user_birthplace']));
/*
 * 用户同意
 */
...
echo '</div>'.K_NEWLINE;
//echo '<script src="../../shared/jscripts/Calendar.js"/>';
//echo '<script src="../../shared/jscripts/calendar-setup.js"/>';
require_once('../code/tce_page_footer.php');
