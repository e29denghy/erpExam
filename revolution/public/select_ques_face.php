...
//		echo "test_duration:".$test_duration;
		if(!isset($_SESSION['exam_duration'])){
			$_SESSION['exam_duration']=array();
			$arr=array();
			array_push($arr,$t_face_exam_id);
			array_push($arr,time()+($test_duration*60));
			array_push($_SESSION['exam_duration'],$arr);
			$examtime=time()+($test_duration*60);
			$_SESSION['exam_nowTime']=$examtime;
//			echo "11：".$examtime;
		}else{
			if(isset($t_face_exam_id)){
				for($i=0;$i<count($_SESSION['exam_duration']);$i++){
					if($_SESSION['exam_duration'][$i][0]==$t_face_exam_id){
//						echo "21:".$_SESSION['exam_duration'][$i][1];
						$examtime=$_SESSION['exam_duration'][$i][1];
						$_SESSION['exam_nowTime']=$examtime;
						$flag=1;
					}
				}
			...
			if(!isset($flag)){
				$arr_new=array();
				array_push($arr_new,$t_face_exam_id);
				array_push($arr_new,time()+($test_duration*60));
				array_push($_SESSION['exam_duration'],$arr_new);
				$examtime=time()+($test_duration*60);
				$_SESSION['exam_nowTime']=$examtime;
//				echo "22:".$_SESSION['exam_duration'][$i][1];
			}
		}
	}else{
//		echo "flash:";
		$examtime=$_SESSION['exam_nowTime'];
	}
    ...
    <div class="templatePreview">
        <div class="exam_border">
            ...
            <div class="exam_page">
                                ...
                                foreach ($res_question_paper as $key => $value) {
                                    if($res_question_paper[$key]["is_flow"]=='0'){
                                        if($papers_questions_id==$res_question_paper[$key]["papers_questions_id"]){
                                            echo '<span class="on_over"><a href="select_ques_face.php?examination_log_id='.$examination_log_id.'&papers_questions_id='.$res_question_paper[$key]['papers_questions_id'].'">'.$res_question_paper[$key]["papers_questions_id"].'</a></span>';
                                        }else{
                                            echo '<span><a href="select_ques_face.php?examination_log_id='.$examination_log_id.'&papers_questions_id='.$res_question_paper[$key]['papers_questions_id'].'">'.$res_question_paper[$key]["papers_questions_id"].'</a></span>';
                                        }
                                    }
                                }
                ...
                echo '<input type="button" value="交卷" name="交卷" id="submit_test"/>';
				//echo '<input type="button" name="add_tip" value="附加题" id="add_tip"/>';
                echo '</form>';          
                echo '<div class="clear"></div>';
                    echo '</div> ';
                echo '</div>';
                ...
                <div class="exam_content">
                    ...
                    foreach ($res_question_paper as $key => $value) {
                        ...
                        <tr>
                            ...
                            <td rowspan="3"  style="width:10px">
                        		<div class="addtion_Bbox" style="width:10px;overflow:visible;">
                        			<input type="button" value="附加题" name="addition_tag" size="12" id=""/>
                        		</div>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <div class="uploadFlowAnsw">
                          <input type="button" value="提交流程" id="upFlowAnsw"/>
                        </div>
                        ...
                        <div class="save_answer_panel">
                            <input type="button" id="save_answer" value ="保存"/> 
                        </div>       
                        ';
		...
		echo '<input type="hidden" name="examtime" id="examtime" value="'.$examtime.'" />'.K_NEWLINE;
...
<script type="text/javascript">
    ...
    var max_step=[1,1];
    var examMode="<?php echo $examMode; ?>";

	function call_next_page(page_now, total_num) {
		page_now++;
		if (page_now <= total_num) {
			window.location.href = "select_ques_face.php?examination_log_id=" + examination_log_id + "&papers_questions_id=" + page_now;
		}
	}
...
<script src="../share/jscripts/exam_interface_examing.js"></script>