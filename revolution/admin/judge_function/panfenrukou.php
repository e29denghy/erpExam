...
foreach($arr_group_id as $k1=>$v1){
  ...
  if(!isset($arr_examination_log_id))
{
...
foreach($arr_examination_log_id as $k3=>$v3){//循环取每一个考生,$v3存有examination_log_id,papers_id,users_id    
...
try{
	...
	{  
		...
		echo "<script>alert('该考试考生".$v3."答案不存在，请确认！')</script>";
		break;
	}
	//添加返回框提示用户
 ...
 if(empty($stu_standard_answer))
{
 ...
 break;
  ...
  //  var_dump($v9);
	      if(isset($v9))
	try{
		onequestionpanfen($v9,$stu_standard_answer,$b1,$user_answer,$v3,$conn_db,$exam_inf,$nums_key_option,$suanzi,$single_add,$multiply_add,$examination_log_id);	  
		  }
		  catch(Exception $e)
		 {
		  if($e->getMessage()=="2")
...
catch(Exception $e)
{
 ...
 $str_insert_error="insert into flow_judge_error values (null,".$exam_id.",".$v3['papers_id'].",null,".$type.",".$v3['users_id'].",null,null,null,null)";
