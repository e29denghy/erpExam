	 ...
	 for($x=0;$x<$nums_id;$x++)
	 {
	  if($single_concept[$x]!=null)
	   ...
	   $singlecon_id=$single_concept[$x]['paperquestions_id'];
        ...
        $str_one_question_score_li="select score from flow_papers_questions where papers_questions_id=".$singlecon_id." and papers_id=".$v3['papers_id'];
	...
	foreach($all_mulquestion_id as $kid=>$vid)//对每一个ID进行扫描三个表，得出该ID在每个表的个数，从而判分。
	  {
		...
		if($vid!=null)//这里是对每一道题分别判分，所以每次选项数量都要重新计算。
		{
         ...
         $str_one_question_score_limul="select score from flow_papers_questions where papers_questions_id=".$vid['paperquestions_id']." and papers_id=".$v3['papers_id'];
