...
//				print_r($v9);
//				
				$str_insert_flow_exam_judge="insert into flow_exam_judge_oncourse values(".$examlog_useransw_id.",".$judge_repeat_id.",".$branch_id.",".$step_id.",".$judge_score.",'".$suanzi."','".$str_judge_remark."') on duplicate key update score=".$judge_score.",judge_remark='".$str_judge_remark."'";
				//echo $str_insert_flow_exam_judge;
				$conn_db->execute_dml($str_insert_flow_exam_judge);
			}
		 }//if(isset($useranswer_step))结束
return array("0"=>$v9,"1"=>$answer_sequence);
		 }//if(isset($v9))的括号
...
function onequestionpanfenOncourse($v9,$stu_standard_answer,$b1,$user_answer,
$v3,$conn_db,$exam_inf,$nums_key_option,$suanzi,
$single_add,$multiply_add,$examination_log_id,$judge_repeat_id)
{
		  foreach($stu_standard_answer as $k10=>$v10)//这个循环取出某个问题ID的所有答案
		   {
		      if(!empty($v10)&&$v10['papers_questions_id']==$v9)
		  ...
		  foreach($user_answer as $k11=>$v11)
		   {//这个循环每次取出1个问题ID的一个步骤,一条考生答案表记录
                if(!empty($v11)&&$v11['papers_question_id']==$v9)
			  {
				  ...
				  $a2[]=$v11['usersansw_steps_id'];//存放该考生答案步骤的ID
//				  print_r($one_question_allanswer);
				  $result_match[]=match_answer($v11,$one_question_allanswer,$nums_key_option,$max);
