...
/**
 * return:返回true 如果teacher_id 是user_id的任课老师
 * $user_id:考生id
 * $teacher_id:任课老师
 */
function F_isUserOfTeacher($user_id, $teacher_id) {
	global $l, $db;
	require_once('../config/tce_config.php');
	$sql = "select * from tce_users where user_id ='{$user_id}' and teacher_id ='{$teacher_id}'";
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			return true;
		}
...
/**
 * return:返回true 如果$class_id 是$user_id的班级id
 * $user_id:考生id
 * $class_id:任课老师
 */
function F_isUserOnClass($user_id, $class_id) {
	global $l, $db;
	require_once('../config/tce_config.php');
	$sql = "select * from tce_users where user_id ='{$user_id}' and class_id ='{$class_id}'";
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			return true;
		}
