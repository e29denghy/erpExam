		...
		if (!isset($user_id) OR empty($user_id)) {
			...
			if ($r = F_db_query($sql, $db)) {
				if ($m = F_db_fetch_array($r)) {
					...
					$user_class_id=$m['user_otpkey'];
					$user_teacher_id=$m['teacher_id'];
					$college=$m['college'];
				} else {
					$user_regdate = '';
					$user_ip = '';
...
//echo getFormRowTextInput('user_otpkey', $l['w_otpkey'], $l['h_otpkey'], '', $user_otpkey, '', 255, false, false, false);
	...
	echo '<div class="row">'.K_NEWLINE;
	echo '<span class="label">'.K_NEWLINE;
	echo '<label for="teacher_id">'.'任课老师'.'</label>'.K_NEWLINE;
	echo '</span>'.K_NEWLINE;
	echo '<span class="formw">'.K_NEWLINE;
	echo '<select name="teacher_id" id="teacher_id">'.K_NEWLINE;
	$sql = 'SELECT * FROM flow_teachers order by flow_teachers_id';
	if ($r = F_db_query($sql, $db)) { 
		while($m = F_db_fetch_array($r)) {
			echo '<option value="'.$m['flow_teachers_id'].'"';
			if (F_isUserOfTeacher($user_id, $m['flow_teachers_id'])) {
				echo ' selected="selected"';
				$m['flow_teachers_name'] = '* '.$m['flow_teachers_name'];
			}
			echo '>'.htmlspecialchars($m['flow_teachers_name'], ENT_NOQUOTES, $l['a_meta_charset']).'</option>'.K_NEWLINE;
		}
	} else {
		echo '</select></span></div>'.K_NEWLINE;
		F_display_db_error();
	...
	echo '<div class="row">'.K_NEWLINE;
	echo '<span class="label">'.K_NEWLINE;
	echo '<label for="class_id">'.'班级'.'</label>'.K_NEWLINE;
	echo '</span>'.K_NEWLINE;
	echo '<span class="formw">'.K_NEWLINE;
	echo '<select name="class_id" id="class_id">'.K_NEWLINE;
	$sql = 'SELECT * FROM flow_class order by class_id';
	if ($r = F_db_query($sql, $db)) {
		while($m = F_db_fetch_array($r)) {
			echo '<option value="'.$m['class_id'].'"';
			if (F_isUserOnClass($user_id, $m['class_id'])) {
				echo ' selected="selected"';
				$m['class_name'] = '* '.$m['class_name'];
			}
			echo '>'.htmlspecialchars($m['class_name'], ENT_NOQUOTES, $l['a_meta_charset']).'</option>'.K_NEWLINE;
		}
	} else {
		echo '</select></span></div>'.K_NEWLINE;
		F_display_db_error();
...
echo getFormRowTextInput('college', "学校", "学校", '', $college, '', 255, false, false, false);

// display QR-Code for Google authenticator
if (!empty($user_otpkey) && false) {
